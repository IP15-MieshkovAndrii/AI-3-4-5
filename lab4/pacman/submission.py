  ...
  def getAction(self, gameState: GameState) -> str:
    ...
    def maxValue(state, depth, alpha, beta):
      ...
      for action in actions:
        ...
        v = max(v, alphaBeta(successor, depth, alpha, beta, 1))
        if v >= beta:
          return v
        alpha = max(alpha, v)
      return v
    ...
    def minValue(state, depth, alpha, beta, agentIndex):
      ...
      for action in actions:
        ...
        v = min(v, alphaBeta(successor, nextDepth, alpha, beta, nextAgent))
        if v <= alpha:
          return v
        beta = min(beta, v)
      return v
    ...
    for action in gameState.getLegalActions(0):
      ...
      score = alphaBeta(successor, 0, alpha, beta, 1)
      if score > bestScore:
        bestScore = score
        bestAction = action
...
class ExpectimaxAgent(MultiAgentSearchAgent):
  ...
  def getAction(self, gameState: GameState) -> str:
    ...
    def expValue(state, depth, agentIndex):
        ...
        for action in actions:
            successor = state.generateSuccessor(agentIndex, action)
            v += prob * expectimax(successor, nextDepth, nextAgent)
        return v
